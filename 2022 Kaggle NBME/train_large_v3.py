...
class CFG:
    ...
    model_pre = "nbme_deberta_v3_large_pretrain_new"
...
def get_results_test(char_probs, th=0.5, labels=None, texts=None, df_in=None,predictions=None):
    results = []
    ann = df_in["annotation"].values
    location = df_in["location"].values
    for idx, char_prob in enumerate(char_probs):

        result = np.where(char_prob >= th)[0] + 1

        target = np.zeros(len(char_prob))
        target[result] = 1
        ...
        if len(result) > 0:
            for r in result:
                ...
                if pos_b > 0:
                    ...
                    for token in encoded.encodings[0].tokens:
                        if txt_a in token and txt_b in token:
                            r[0] = r[0] - 1
                            break
                result_new.append(r)
        ...
        result = result_new
        result_list = [f"{min(r)} {max(r)}" for r in result]
        # result_new = result.copy()
        # if len(result_new) > 0:
        #     for offset_mapping in encoded['offset_mapping']:
        #         start = offset_mapping[0]
        #         end = offset_mapping[1]
        #         if start < result_new[0][0] and result_new[0][0]< end:
        #             result_new[0][0] = start
        #
        #         if start < result_new[-1][-1] and result_new[-1][-1]< end:
        #             result_new[-1][-1] = end
        #
        #     result_list_new = [f"{min(r)} {max(r)}" for r in result_new]
        #     targ = ann[idx]
        #     for r in result_new:
        #         pos_b = min(r)
        #         pos_e = max(r)
        #         tmp_str = te[pos_b:pos_e]
        #         print(tmp_str)

        # predictions_tmp = predictions[idx]
        # results_test = np.zeros(len(te))
        # for idx, (offset_mapping, pred) in enumerate(zip(encoded['offset_mapping'], predictions_tmp)):
        #     start = offset_mapping[0]
        #     end = offset_mapping[1]
        #     results_test[start:end] = pred

        if result_list != lo_tmp:
            targ = ann[idx]
            for r in result:
                pos_b = min(r)
                pos_e = max(r)
...
# features = pd.read_csv(Root_path + 'features_with_meanings.csv')
# features = preprocess_features(features)
# features['feature_text'] = features.apply(lambda row: row['feature_text'] + '[SEP]meaning: ' + row['meaning'], axis=1)
# features.drop(['meaning'], axis=1, inplace=True)
...
if Add_fake:
    ...
    # Fold_f = StratifiedKFold(n_splits=CFG.n_fold)
    # groups_f = train_fake['case_num'].values
    # for n, (train_index, val_index) in enumerate(Fold_f.split(train_fake['location'], groups_f)):
    #     train_fake.loc[val_index, 'fold'] = int(n)
    # train_fake['fold'] = train_fake['fold'].astype(int)
    # display(train_fake.groupby('fold').size())
    #
    ...
    # train_fake['annotation_length'] = train_fake['annotation'].apply(len)
    #
    # train_fake.to_csv('889_1000pn_new_case_stratif.csv', index=False)
...
class CustomModel(nn.Module):
    ...
    # def forward(self, inputs, y=None, loss_fn=None):
    #     feature = self.feature(inputs)
    #     # output = self.fc(self.fc_dropout(feature))
    #     if len(self.dropouts) == 0:
    #         out = self.fc(self.fc_dropout(feature))
    #         if loss_fn is not None:
    #             loss = loss_fn(out.view(-1, 1), y.view(-1, 1))
    #             return out, loss
    #         return out, None
    #     else:
    #         for i, dropout in enumerate(self.dropouts):
    #             if i == 0:
    #                 out = self.fc(dropout(feature))
    #                 if loss_fn is not None:
    #                     loss = loss_fn(out.view(-1, 1), y.view(-1, 1))
    #             else:
    #                 temp_out = self.fc(dropout(feature))
    #                 out = out + temp_out
    #                 if loss_fn is not None:
    #                     loss = loss + loss_fn(temp_out.view(-1, 1), y.view(-1, 1))
    #         if loss_fn is not None:
    #             loss = torch.masked_select(loss, y.view(-1, 1) != -1).mean()
...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device, fgm, pgd, ema_inst,
    ...
    for step, (inputs, labels) in enumerate(train_loader):
        ...
        if (step + 1) % CFG.gradient_accumulation_steps == 0:
            ...
            if use_ema:
                ema_inst.update(model.parameters())
        ...
        if (step+1) % save_step == 0 and epoch > 0:
            if use_ema:
                ema_inst.store(model.parameters())
                ema_inst.copy_to(model.parameters())
            # eval
            ...
            results = get_results(char_probs, th=0.5, texts=valid_texts)
            ...
            if use_ema:
                ema_inst.restore(model.parameters())
...
def train_loop(folds, fold):
    ...
    ema_inst = EMA(model.parameters(), decay=0.999)
    ...
    for epoch in range(CFG.epochs):
        ...
        # train
        avg_loss, new_score = train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device, fgm, pgd, ema_inst,
                            valid_loader,valid_folds,valid_texts,valid_labels, best_score)
...
if __name__ == '__main__':
    ...
    def get_result(oof_df,oof_df_pre):
        ...
        #results = get_results_test(char_probs, 0.5, labels, oof_df['pn_history'].values, oof_df, predictions)
        results = get_results(char_probs, th=0.5,texts=oof_df['pn_history'].values)
        preds = get_predictions(results)
